...
data = pickle.load(open( "../data/adelie_data.pkl", "rb" ))
id = 'charcot'
site_name = 'Port Charcot'
...
# # # SYNTHETIC DATA
# # PART 1: LOAD THE DATA
# simulation = np.load('adelie_simulated_data.npz')
# id = 'synthetic_adelie'
# y = simulation['data']
...
# Prior parameters
alpha_juv0 = 3              # Juvenile survival prior - beta prior (mean of 0.43)
beta_juv0 = 3
alpha_adu0 = 3              # Adult survival prior - beta prior (mean of 0.82)
beta_adu0 = 3
mu_int_pb0 = 0              # Breeding success (intercept in logit) - Normal(0,10)
var_int_pb0 = 10
int_diff_low = 0            # Breeding success (intercept in logit) - Normal(0,10)
int_diff_high = 3 - int_diff_low
mu_slope_pb0 = 0.5           # Breeding success (slope in logit) - Normal(0.5, 0.04)
var_slope_pb0 = 0.2**2
alpha_ps = 1                # Probability of a bad year
beta_ps = 9

# Store prior parameters in summary
prior = {'juvenile survival': [alpha_juv0, beta_juv0],
         'adult survival': [alpha_adu0, beta_adu0],
         'reproductive bias (bad)': [mu_int_pb0, var_int_pb0],
         'reproductive bias (good)': [int_diff_low, int_diff_high],
         'reproductive slope': [mu_slope_pb0, var_slope_pb0],
         'switching probability': [alpha_ps, beta_ps]}
...
def log_likelihood_per_sample(input_parameters):
    ...
    log_prior += sp.uniform.logpdf(z[3], loc=int_diff_low, scale=int_diff_high) + input_parameters[3]
    log_prior += sp.norm.logpdf(z[4], loc=mu_slope_pb0, scale=np.sqrt(var_slope_pb0)) + input_parameters[4]
    log_prior += sp.beta.logpdf(z[5], alpha_ps, beta_ps) + log_jacobian_sigmoid(input_parameters[5])
    # Initialize log joint as log prior
    log_joint = log_prior
    # Create the model (assuming the noise variances are known)
    regimes = [adelie.AgeStructuredModel(psi_juv=z[0], psi_adu=z[1], alpha_r=z[2], beta_r=z[4], nstage=5),
               adelie.AgeStructuredModel(psi_juv=z[0], psi_adu=z[1], alpha_r=z[2]+z[3], beta_r=z[4], nstage=5)]
    ...
    init_particles = np.array([initial_states]).T+np.random.randint(low=x_low, high=x_high, size=(2*num_stages-2, num_particles))
...
if __name__ == '__main__':
    ...
    for j in range(D):
        ...
        mu_init[j, 0] = np.random.uniform(0.4, 0.6)
        mu_init[j, 0] = np.log(mu_init[j, 0]/(1-mu_init[j, 0]))
        sig_init[j, 0, 0] = 0.05**2
        # Prior proposal parameters for adult survival
        mu_init[j, 1] = np.random.uniform(0.75, 0.9)
        mu_init[j, 1] = np.log(mu_init[j, 1]/(1-mu_init[j, 1]))
        sig_init[j, 1, 1] = 0.05**2
        # Prior proposal parameters for logit intercept (bad year)
        mu_init[j, 2] = np.random.uniform(-2, 0)
        sig_init[j, 2, 2] = 0.3**2
        # Prior proposal parameters for difference in logit intercept
    ...
    state_labels = ['$S_{1,t}$', '$S_{2, t}$', '$S_{3, t}$', '$S_{4, t}$', '$S_{5, t}$', '$C_{3,t}$', '$C_{4, t}$',
                    '$C_{5, t}$']
    parameter_labels = ['Juvenile Survival', 'Adult Survival', 'Reproductive Rate (Bad)', 'Reproductive Rate (Diff)',
                        'Reproductive Rate (Logit Slope)', 'Probability of Bad Year']
